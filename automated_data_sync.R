...
read_one_cache <- function(fp) {
  df <- suppressMessages(readr::read_csv(
    fp,
    col_types = readr::cols(.default = readr::col_character())
  ))

  path_st <- infer_session_from_path(fp)
...
build_pitch_cache <- function() {
  cat("Building cached data for Shiny app startup...\n")
  all_csvs <- list.files(LOCAL_DATA_DIR, pattern = "\\.csv$", recursive = TRUE, full.names = TRUE)
  all_csvs <- all_csvs[grepl("practice|v3", tolower(all_csvs))]

  if (!length(all_csvs)) {
    cat("No CSV files found for cache build.\n")
    return(FALSE)
  }

  cache_list <- purrr::map(all_csvs, function(fp) {
    tryCatch(read_one_cache(fp), error = function(e) {
      cat("  Skipping", fp, "-", e$message, "\n")
      NULL
    })
  })

  cache_df <- purrr::map_dfr(cache_list, function(x) x)

  cache_path <- file.path(LOCAL_DATA_DIR, "pitch_data_cache_raw.qs")
  qs::qsave(cache_df, cache_path, preset = "high")
  cat("Saved cache to", cache_path, "with", nrow(cache_df), "rows\n")
  TRUE
}
...
main_sync <- function() {
  ...
  # Always refresh the pitch data cache so the Shiny app can load quickly
  build_pitch_cache()
