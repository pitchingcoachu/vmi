...
parse_date_flex <- function(x, pivot = 1970L) {
  ...
  # Excel serials (rough range guard)
  serial <- suppressWarnings(as.numeric(x_chr))
  is_serial <- !is.na(serial) & serial > 20000 & serial < 90000
  ...
  orders <- c(
    ...
    "b d, Y", "d b, Y", "B d, Y", "d B, Y",
    "Y-m-d HMS", "Y/m/d HMS", "Y-m-d HM", "Y/m/d HM",
    "m/d/Y HMS", "m/d/y HMS", "m/d/Y HM", "m/d/y HM",
    "d/m/Y HMS", "d/m/y HMS", "d/m/Y HM", "d/m/y HM",
    "b d Y HMS", "d b Y HMS", "b d Y HM", "d b Y HM"
  )
  ...
  if (any(is_serial)) {
    parsed[is_serial] <- as.Date(serial[is_serial], origin = "1899-12-30")
  }

  yrs <- suppressWarnings(lubridate::year(dt))
  idx <- !is.na(yrs) & yrs < pivot
  if (any(idx)) parsed[idx] <- as.Date(dt[idx] + lubridate::years(100))
...
library(readr)
library(stringr)   # explicit string helpers
...
qs_cache  <- file.path(data_parent, "pitch_data_cache_raw.qs")
rds_cache <- file.path(data_parent, "pitch_data_cache_raw.rds")

if (file.exists(qs_cache)) {
  pitch_data <- qs::qread(qs_cache)
} else if (file.exists(rds_cache)) {
  pitch_data <- readRDS(rds_cache)
} else {
  pitch_data <- purrr::map_dfr(all_csvs, read_one)
}

if (!"SourceFile" %in% names(pitch_data)) {
  pitch_data$SourceFile <- NA_character_
}

# Map folder → SessionType
infer_session_from_path <- function(fp) {
  fpl <- tolower(fp)
  base <- tolower(basename(fp))
  if (grepl("[/\\\\]practice[/\\\\]", fpl) || grepl("practice", base)) {
    "Bullpen"
  } else if (grepl("[/\\\\]v3[/\\\\]", fpl) || grepl("^v3", base)) {
    "Live"
  } else {
    "Live"
  }
...
extract_folder_date <- function(fp) {
  ...
  if (nrow(m)) {
    return(as.Date(paste(m[nrow(m),2], m[nrow(m),3], m[nrow(m),4], sep = "-")))
  }
  # fallback: capture YYYY-MM-DD or YYYY_MM_DD patterns in the filename
  base <- basename(fp)
  m2 <- stringr::str_match(base, "(20\\d{2})[-_](0[1-9]|1[0-2])[-_](0[1-9]|[12]\\d|3[01])")
  if (!all(is.na(m2))) {
    return(as.Date(paste(m2[2], m2[3], m2[4], sep = "-")))
  }
  NA
}
...
read_one <- function(fp) {
  ...
  if (all(is.na(df$Date))) {
    ...
    if (!is.na(folder_date)) {
      df$Date <- rep(as.character(folder_date), nrow(df))
    }
  }
  # -------------------------------------------------------------------------------
...
# Dedicated V3 dataset for correlations (full roster, restricted date range)
CORR_V3_DATE_MIN <- as.Date("2025-02-14")
CORR_V3_DATE_MAX <- as.Date("2025-06-22")

corr_v3_data <- pitch_data %>%
  mutate(.source_lower = tolower(as.character(SourceFile))) %>%
  filter(
    !is.na(.source_lower),
    grepl("[/\\\\]v3[/\\\\]", .source_lower) | grepl("^v3_", basename(.source_lower)),
    !grepl("playerpositioning", .source_lower),
    !grepl("unverified", .source_lower),
    SessionType == "Live",
    !is.na(Date),
    Date >= CORR_V3_DATE_MIN,
    Date <= CORR_V3_DATE_MAX
  ) %>%
  dplyr::select(-.source_lower)
  ...
  fluidPage(
    ...
    fluidRow(
      ...
      column(3, class = "correlation-sidebar",
             ...
             div(class = "correlation-controls",
                 dateRangeInput(
                   "corr_date_range", "Date Range:",
                   start = CORR_V3_DATE_MIN,
                   end   = CORR_V3_DATE_MAX,
                   min   = CORR_V3_DATE_MIN,
                   max   = CORR_V3_DATE_MAX,
                   format = "yyyy-mm-dd"
                 )
             ),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (input$corr_domain == "Pitching") {
      players <- sort(unique(na.omit(as.character(base_data$Pitcher))))
    } else if (input$corr_domain == "Hitting") {
      if (nzchar(tc)) {
        base_data <- base_data %>% dplyr::filter(BatterTeam %in% codes_for(tc))
      }
      players <- sort(unique(na.omit(as.character(base_data$Batter))))
    } else if (input$corr_domain == "Catching") {
      if (nzchar(tc)) {
        base_data <- base_data %>% dplyr::filter(PitcherTeam %in% codes_for(tc))
      }
      players <- sort(unique(na.omit(as.character(base_data$Catcher))))
    } else {
      players <- character(0)
    }
  ...
  observe({
    ...
    if (input$corr_domain == "Pitching") {
      # Get unique pitch types from the correlations dataset
      available_pitches <- sort(unique(na.omit(corr_v3_data$TaggedPitchType)))
  ...
  corr_data <- eventReactive(input$corr_analyze, {
    ...
    if (input$corr_domain == "Pitching") {
      data <- corr_v3_data %>% dplyr::filter(!is.na(Pitcher))
      player_col <- "Pitcher"
      cat("Using corr_v3_data for pitching, initial rows:", nrow(data), "\n")
    } else if (input$corr_domain == "Hitting") {
      data <- corr_v3_data %>% dplyr::filter(!is.na(Batter))
      player_col <- "Batter"
      cat("Using corr_v3_data for hitting, initial rows:", nrow(data), "\n")
    } else if (input$corr_domain == "Catching") {
      data <- corr_v3_data %>% dplyr::filter(!is.na(Catcher))
      player_col <- "Catcher"
      cat("Using corr_v3_data for catching, initial rows:", nrow(data), "\n")
    } else {
      cat("Invalid domain:", input$corr_domain, "\n")
      return(NULL)
    }
    
    # Apply team filtering for non-pitching domains (pitching uses full roster)
    if (input$corr_domain != "Pitching") {
      tc <- get0("TEAM_CODE", ifnotfound = "")
      if (nzchar(tc)) {
